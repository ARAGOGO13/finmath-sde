...
// =============================================================================
// Универсальные функции визуализации результатов Монте-Карло
//
// Файл содержит вспомогательные функции для построения графиков траекторий,
// математических ожиданий, дисперсий и анализа сходимости численных схем.
// Функции используют библиотеку Matplot++ (matplot) и сохраняют графики
// в формате PDF.
//
// Основные возможности:
//   • вычисление выборочных среднего и дисперсии по набору траекторий
//   • рисование полупрозрачных путей с наложением теоретических и МК средних
//   • построение графиков дисперсии
//   • визуализация сильной сходимости схем Эйлера и Мильштейна
//   • комплексная визуализация для процессов GBM, InverseGBM, Vasicek, CIR
//   • визуализация пуассоновских процессов
// =============================================================================


// -----------------------------------------------------------------------------
// Вычисление выборочного среднего и дисперсии по траекториям Монте-Карло
// -----------------------------------------------------------------------------
// Принимает матрицу траекторий paths размера (M × N), где:
//   M – число траекторий
//   N – число временных точек
// Заполняет векторы mc_mean и mc_var длиной N, содержащие среднее и дисперсию
// по ансамблю траекторий в каждый момент времени.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Вспомогательная функция для рисования траекторий и среднего
// -----------------------------------------------------------------------------
// Рисует на переданных осях ax:
//   • полупрозрачные траектории (цвет задаётся через path_color)
//   • Монте-Карло среднее (красный пунктир)
//   • теоретическое среднее (чёрная сплошная линия)
//
// Параметры:
//   ax          – указатель на оси Matplot
//   time        – временная сетка
//   paths       – матрица траекторий (первые num_paths_to_plot используются)
//   mc_mean     – вектор МК средних
//   th_mean     – вектор теоретических средних
//   title_str   – заголовок графика
//   ylabel_str  – подпись оси Y
//   path_color  – цвет и прозрачность траекторий в формате {R,G,B,alpha}
//   y_lim       – опциональный диапазон оси Y {min, max}. Если max ≤ min,
//                 диапазон выбирается автоматически
//
// Использует трюк с двойной отрисовкой: сначала рисуются прокси-линии для
// легенды, затем все траектории, затем средние линии повторно поверх путей.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Вспомогательная функция для рисования дисперсии
// -----------------------------------------------------------------------------
// Рисует на осях ax:
//   • Монте-Карло дисперсию (красный пунктир)
//   • теоретическую дисперсию (чёрная сплошная линия)
//
// Параметры:
//   ax         – указатель на оси Matplot
//   time       – временная сетка
//   mc_var     – вектор МК дисперсий
//   th_var     – вектор теоретических дисперсий
//   title_str  – заголовок графика
//   ylabel_str – подпись оси Y
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Комплексная визуализация GBM и InverseGBM
// -----------------------------------------------------------------------------
// Сохраняет четыре PDF-файла:
//   • E[S_t] и Var[S_t] для GBM
//   • E[U_t] и Var[U_t] для InverseGBM (U = 1/S)
//
// Параметры:
//   time        – временная сетка
//   gbm_paths   – траектории GBM (первые N_paths для графиков)
//   gbm_mean_th, gbm_var_th – теоретические моменты GBM
//   inv_paths   – траектории InverseGBM
//   inv_mean_th, inv_var_th – теоретические моменты InverseGBM
//   sde_gbm, sde_inv – строки с уравнениями СДУ для заголовков
//   inv_mean_ylim – фиксированный диапазон Y для графика E[U_t]
//   base        – базовое имя выходных файлов (добавляются суффиксы)
// -----------------------------------------------------------------------------
...
// Визуализация сильной сходимости схем Эйлера и Мильштейна
// -----------------------------------------------------------------------------
// Строит график в логарифмическом масштабе: log(ошибка) от log(N),
// где N – число шагов дискретизации. Наклоны теоретических прямых:
//   • -0.5 для Эйлера (порядок сильной сходимости 0.5)
//   • -1.0 для Мильштейна (порядок сильной сходимости 1.0)
//
// Параметры:
//   res      – структура с результатами анализа сходимости
...
// -----------------------------------------------------------------------------
// Сохраняет два PDF-файла: график среднего и график дисперсии.
// Использует общие функции draw_paths_mean и draw_var.
//
// Параметры:
//   time        – временная сетка
//   paths       – траектории процесса
//   mean_th, var_th – теоретические моменты
//   model_name  – имя модели (для заголовка)
//   sde_str     – строка с уравнением СДУ
//   mean_ylim   – фиксированный диапазон Y для графика среднего
//   base        – базовое имя выходных файлов
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Сохраняет три PDF-файла:
//   • примеры траекторий для трёх интенсивностей λ (ступенчатые графики)
//   • математическое ожидание E[N_t] (МК пунктир, теория λt сплошная)
//   • дисперсия Var[N_t] (МК пунктир, теория λt сплошная)
//
// Параметры:
//   time_grid  – временная сетка
//   all_paths  – вектор (по λ) матриц траекторий
//   lambdas    – вектор значений интенсивности λ
//   base       – базовое имя выходных файлов
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Использует жёстко заданные параметры процессов, чтобы обеспечить единый
// масштаб при визуализации E[U_t] (InvGBM), E[Vasicek] и E[CIR].
// Возвращает std::array<double,2> {y_min, y_max}.
// -----------------------------------------------------------------------------
inline std::array<double,2> compute_shared_mean_ylim(double T) {
    // Параметры InverseGBM: U0=1/100, sigma=0.20, mu_star = sigma^2 - 0.05
    ...
    // Параметры Vasicek / CIR: theta=2.0, mu=0.05, sigma=0.02, X0=0.03
